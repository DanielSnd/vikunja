...
func TestConvertWekanToVikunja(t *testing.T) {
	...
	assert.Equal(t, "f87168", task1.Labels[0].HexColor) // red mapped
	...
	assert.Equal(t, "This is urgent", task1.Comments[0].Comment)
	...
	assert.Equal(t, "4bce97", task2.Labels[0].HexColor) // green mapped
...
func TestConvertWekanLabelColorMapping(t *testing.T) {
	...
	assert.Equal(t, "f87168", labels[0].HexColor) // red
	assert.Equal(t, "4bce97", labels[1].HexColor) // green
	assert.Equal(t, "b8c2cc", labels[2].HexColor) // fallback
}
