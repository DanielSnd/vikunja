...
// WeKan uses the same color names as Trello.
...
func convertWekanToVikunja(board *wekanBoard) []*models.ProjectWithTasksAndBuckets {
	...
	for _, card := range board.Cards {
		...
		for _, labelID := range card.LabelIDs {
			...
			hexColor, colorExists := wekanColorMap[label.Color]
			if !colorExists {
		...
		if checklists, ok := checklistsByCardID[card.ID]; ok {
			for _, cl := range checklists {
				...
				if items, ok := checklistItemsByChecklistID[cl.ID]; ok {
...
// @Failure 500 {object} models.Message "Internal server error"
// @Router /migration/wekan/migrate [post]
